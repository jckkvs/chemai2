...
def _render_heatmap(ax, trace) -> None:
    ...
    if x_labels is not None and len(x_labels) <= 40:
        ...
        ax.set_xticklabels([str(l) for l in x_labels], rotation=45, ha="right", fontsize=8)
...
def _render_box(ax, trace, color: str) -> None:
    ...
    if y is None or len(y) == 0:
        return
    name = trace.name or ""
    ax.boxplot(y, labels=[name] if name else None, patch_artist=True,
...
def _render_violin(ax, trace, color: str) -> None:
    ...
    try:
        ...
        ax.boxplot(y, labels=[name] if name else None)
