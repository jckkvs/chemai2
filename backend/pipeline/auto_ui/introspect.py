...
class EstimatorIntrospector:
    ...
    def _annotation_to_schema(annotation) -> dict:
        ...
        if origin is None:
            if annotation == int: return {"type": "integer"}
            elif annotation == float: return {"type": "number"}
            elif annotation == bool: return {"type": "boolean"}
            elif annotation == str: return {"type": "string"}
        elif origin == Union:
            ...
            if len(args) == 1:
                ...
                return {"type": "number"}
        elif origin == list or getattr(annotation, "_name", "") == "List":
