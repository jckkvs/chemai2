...
def get_smiles_hover_html(smiles: str, img_size: int = 200) -> str:
    """
    SMILES構造式＋すぐ計算できる特徴量を含むHTMLを生成。
    20260429.txtの要件：「SMILESはマウスオーバーすれば、その構造式やすぐ計算できる特徴量がわかるように」
    """
    # 構造式画像
    img_uri = _best_img(smiles, img_size, img_size)
    img_part = ""
    if img_uri:
    ...
    # すぐ計算できる特徴量
    try:
        from backend.chem.smiles_quick_features import get_feature_summary_html
        feature_html = get_feature_summary_html(smiles, img_uri=img_uri, img_size=img_size)
        return feature_html
    except Exception as e:
        # フォールバック：画像のみ
        smiles_short = smiles[:40] + ("..." if len(smiles) > 40 else "")
        return f"""
        <div style="text-align:center; font-family:monospace; font-size:11px; color:#7fffd4; margin-bottom:4px;">
            {smiles_short}
        </div>
        {img_part}
        <div style="border-top:1px solid #4c9be8; padding-top:6px; margin-top:6px; font-size:11px; color:#ef4444;">
            特徴量計算エラー: {str(e)[:50]}
        </div>
        """


def render_smiles_table_with_features(
    df: pd.DataFrame,
    smiles_col: str,
    max_rows: int = 100,
    img_size: int = 220,
    ...
    """
    SMILES列にホバーで2D構造＋すぐ計算できる特徴量を表示するHTMLテーブル。
    20260429.txtの要件完全対応版。
    """
    ...
    for _, row in display_df.iterrows():
        ...
        for col in display_df.columns:
            ...
            if col == smiles_col and val_str:
                # ホバー用の特徴量付きHTML生成
                hover_html = get_smiles_hover_html(val_str, img_size)
                truncated = val_str[:30] + ("..." if len(val_str) > 30 else "")
                # HTMLエスケープ
                import html
                hover_escaped = html.escape(hover_html)
                cells += (
                    f'<td class="smi-cell">'
                    f'<span class="smi-text">{truncated}</span>'
                    f'<div class="smi-popup">{hover_html}</div>'
                    f'</td>'
                )
            else:
