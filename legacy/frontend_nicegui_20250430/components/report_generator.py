...
def render_report_tab(state: dict[str, Any]) -> None:
    ...
    with ui.row().classes("q-gutter-md"):
        def _gen_md():
            # LLM考察を生成
            ai_commentary = _generate_llm_commentary(ar, state)
            md = _generate_markdown_report(ar, state, ai_commentary=ai_commentary)
...
def _generate_markdown_report(ar, state: dict, ai_commentary: str = "") -> str:
    """解析結果からMarkdownレポートを生成する。ai_commentary があればLLM考察を含める。"""
    lines: list[str] = []
...
def _generate_llm_commentary(ar, state: dict) -> str:
    """LLMによる考察・推奨を生成する。失敗時は空文字を返す。"""
    try:
        ...
        task_type = ar.task if hasattr(ar, "task") else state.get("task_type", "regression")
        ...
        results = AnalysisResults(
            ...
            model_comparison=[
                ...
                for k, v in ar.model_scores.items()
            ],
