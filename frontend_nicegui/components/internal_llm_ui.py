...
def render_internal_llm_tab(state: dict) -> None:
    ...
    with ui.card().classes("full-width q-pa-sm q-mb-sm").style(
        ...
        model_sel = ui.select(
            model_options,
            value=_ui_state["model_id"],
            label="使用するモデル",
            on_change=lambda e: _on_model_change(e.value, _ui_state, cfg, dl_status_label),
        ).props("dense outlined").classes("full-width q-mb-xs")

        # モデル説明
        desc_label = ui.label("").classes("text-grey-5 text-caption q-mb-xs")
        _update_desc(desc_label, _ui_state["model_id"])
        ...
        def _on_model_change(new_id: str, st: dict, c: dict, lbl) -> None:
            st["model_id"] = new_id
            c["model_id"] = new_id
            save_hf_config(c)
            _update_desc(desc_label, new_id)
            _update_download_status()
...
def _update_desc(label, model_id: str) -> None:
...
async def _do_download(ui_state: dict, btn, status_lbl, progress_bar) -> None:
    """モデルダウンロードを実行する。"""
    from backend.llm.providers.hf_provider import (
        download_model_async, get_download_progress, is_model_downloaded
    )
    from nicegui import run as ng_run

    mid = ui_state["model_id"]
    tok = ui_state["token"]

    if not tok:
        ui.notify("HuggingFace トークンを先に保存してください", type="warning")
        return
