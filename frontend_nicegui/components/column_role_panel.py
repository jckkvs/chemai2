...
def render_column_role_panel(state: dict[str, Any]):
    ...
    if "column_roles" not in state:
        ...
        for col in df.columns:
            if col == target: state["column_roles"][col] = "target"
            elif col in excludes: state["column_roles"][col] = "exclude"
            else: state["column_roles"][col] = "feature"
        ...
        if e.column == "role":
            ...
            if new_role == "target":
                ...
                if col_name not in state["exclude_cols"]:
                    ...
                    if state.get("target_col") == col_name: state["target_col"] = ""
            else: # feature
