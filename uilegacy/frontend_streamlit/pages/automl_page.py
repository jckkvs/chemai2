...
from backend.ml.cv_factory import CVStrategyFactory
...
def _render_cv_settings(
    ...
    """
    CV 設定 UI を描画し、(cv_key, cv_folds, cv_groups_col) を返す。
    run_config がある場合はそれを優先し、UI での上書きを許可する。
    """
    # 既定値
    _cv_key = "auto"
    _cv_folds = 5
    ...
    if run_config:
        _cv_key = run_config.get("cv_key", "auto")
        _cv_folds = run_config.get("cv_folds", 5)
        _manual_group_col = run_config.get("cv_groups_col")
        if _manual_group_col and _manual_group_col in df.columns:
            _cv_groups_col = _manual_group_col
        # リーケージ検出結果の反映
        _leakage_groups = run_config.get("leakage_group_labels")
        if _leakage_groups is not None and len(_leakage_groups) == len(df):
            df = df.copy()
            df["_leakage_group"] = _leakage_groups
            _cv_groups_col = "_leakage_group"

    # 利用可能な CV ストラテジー一覧
    with st.expander("🔄 CV 設定", expanded=True):
        try:
            available = CVStrategyFactory.get_available_strategies()
        except Exception:
            available = [
                "kfold", "stratified_kfold", "stratified_shuffle",
                "shuffle", "loo", "loogroup", "lop",
                "group_kfold", "group_shuffle", "timeseries", "predefined",
            ]
        cv_options = ["auto（推奨）"] + available

        # LLM 自動推奨
        rec = None
        try:
            X_cv = df.drop(columns=[target_col])
            y_cv = df[target_col]
            task_cv = st.session_state.get("task", "auto")
            if task_cv == "auto":
                if y_cv.dtype == object or y_cv.nunique() <= 10:
                    task_cv = "classification"
                else:
                    task_cv = "regression"
            rec = CVStrategyFactory.recommend_strategy(
                X_cv, y_cv, groups=None, task_type=task_cv)
            if rec and "strategy" in rec:
                st.success(f"🤖 LLM 推奨: **{rec['strategy']}**")
                st.caption(f"理由: {rec['reason']}")
                if rec.get("confidence"):
                    conf_emoji = {"high": "✅", "medium": "⚡", "low": "⚠️"}[rec["confidence"]]
                    st.caption(f"{conf_emoji} 信頼度: {rec['confidence']}")
                if rec.get("alternatives"):
                    st.caption(f"代替案: {', '.join(rec['alternatives'])}")
        except Exception as e:
            st.warning(f"推奨の取得に失敗: {e}")
        ...
        selected_cv = st.selectbox(
            ...
            cv_options,
            index=0,
            key=f"{key_prefix}cv_type_selector",
        )
        _cv_folds = st.slider("CV 分割数", 2, 20, _cv_folds, key=f"{key_prefix}cv_folds_slider")

        # 選択結果の反映
        if selected_cv != "auto（推奨）":
            _cv_key = selected_cv
            st.caption(f"手動選択: {selected_cv}")
        else:
            if rec and "strategy" in rec:
                _cv_key = rec["strategy"]
            else:
                _cv_key = "kfold"

        # グループ列の手動選択（オプション）
        group_cols = [c for c in df.columns if df[c].nunique() < len(df) * 0.5]
        if group_cols:
            sel_group = st.selectbox(
                "グループ列（任意）",
                ["自動"] + group_cols,
                key=f"{key_prefix}cv_group_sel",
            )
            if sel_group != "自動":
                _cv_groups_col = sel_group
                _cv_key = "group_kfold"
                st.caption(f"グループ列 `{sel_group}` を使用して GroupKFold を適用")

    return _cv_key, _cv_folds, _cv_groups_col
...
def render(run_config: dict | None = None) -> None:
    ...
    # ── CV 設定 ────────────────────────────────────────────────
    _cv_key, _cv_folds, _cv_groups_col = _render_cv_settings(
        df, target_col, run_config, key_prefix="main_"
    )
...
<div style="text-align:center; padding:3rem; color:#555;">
  ...
  <div style="margin-top:0.5rem; font-size:0.85rem; color:#666;">
    ...
    # 除外列
    _exclude_cols = adv.get("exclude_cols", []) if not run_config else run_config.get("exclude_cols", [])
    ...
    # Weight列を除外
    _weight_col = adv.get("sample_weight_col") or (run_config or {}).get("sample_weight_col")
    ...
    _smiles_col = st.session_state.get("smiles_col") or (run_config or {}).get("smiles_col")
    _task = st.session_state.get("task", "auto") if not run_config else run_config.get("task", "auto")
    _scaler = adv.get("scaler", "auto") if not run_config else run_config.get("scaler", "auto")
    _models = adv.get("models", []) if not run_config else run_config.get("models", [])
    _timeout = adv.get("timeout", 300) if not run_config else run_config.get("timeout", 300)

    _run_full_pipeline(
        df=_df_for_run,
        target_col=target_col,
        smiles_col=_smiles_col,
        ...
        models=_models,
        timeout=_timeout,
        do_eda=adv.get("do_eda", True) if not run_config else run_config.get("do_eda", True),
        do_prep=adv.get("do_prep", True) if not run_config else run_config.get("do_prep", True),
        do_ml=True,
        do_eval=adv.get("do_eval", True) if not run_config else run_config.get("do_eval", True),
        do_pca=adv.get("do_pca", True) if not run_config else run_config.get("do_pca", True),
        do_shap=adv.get("do_shap", True) if not run_config else run_config.get("do_shap", True),
        selected_descriptors=adv.get("selected_descriptors") if not run_config else run_config.get("selected_descriptors"),
    ...
    *,
    ...
    try:
        ...
        if result.automl_result and result.automl_result.processed_X is not None:
            ...
            # numpy array の場合は DataFrame に戻す（ColumnTransformer のため）
            if not isinstance(X_base, pd.DataFrame):
                try:
                    feat_names = result.automl_result.best_pipeline[:-1].get_feature_names_out()
                except Exception:
                    n_cols = X_base.shape[1] if hasattr(X_base, 'shape') else len(X_base[0])
                    feat_names = [f"feature_{i}" for i in range(n_cols)]
                X_base = pd.DataFrame(
                    X_base, columns=feat_names,
                    index=df.index[:X_base.shape[0]] if hasattr(X_base, 'shape') else None
                )
...
def _show_pipeline_result(pr: PipelineResult) -> None:
    ...
    with tabs[1]:
        ...
        if dr:
            ...
            st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
            st.caption("TypeDetector によって自動で各列の型が判定され、最適なスケーラー/エンコーダーが選定されます。")
        else:
    ...
    with tabs[6]:
        if pr and hasattr(pr, "smiles_correlations") and pr.smiles_correlations:
            ...
            st.caption("SMILESから抽出された各記述子と目的変数とのピアソン相関係数（絶対値でランキング表示）")
    ...
    df_lb = pd.DataFrame([
        {
            ...
            "モデル": k,
            "スコア（平均）": f"{v:.4f}",
            "標準偏差": f"{details.get(k, {}).get('std', 0):.4f}",
            "学習時間(s)": f"{details.get(k, {}).get('fit_time', 0):.2f}",
    ...
    if has_fold_scores:
        with st.expander("📐 CVバイアス推定", expanded=False):
            try:
                ...
                curves = _np.array(fold_curves).T  # (K folds, N models)
